...
const evidenceDir = resolve(root, 'pixelizer-codex-research/evidence/pal-001');
...
const inlineScripts = [...html.matchAll(/<script(?:\s[^>]*)?>([\s\S]*?)<\/script>/gi)]
  .map(m => m[1])
  .filter(s => s.trim());
const cspHash = createHash('sha256').update(inlineScripts[0]).digest('base64');

// Sample test executions
const gbPalette = ['#0F380F', '#306230', '#8BAC0F', '#9BBC0F'];
const pico8Palette = ['#000000', '#1D2B53', '#7E2553', '#008751', '#AB5236', '#5F574F', '#C2C3C7', '#FFF1E8'];
...
const summaryData = {
  itemId: 'PAL-001',
  itemTitle: 'Custom palette 입력·미리보기·검증',
  status: 'DONE',
  generatedAt: new Date().toISOString(),
  cspHash: `sha256-${cspHash}`,
  features: {
    paletteModes: ['auto', 'custom', 'unlimited'],
    parsers: ['hex', 'gpl', 'png'],
    deduplication: 'first-appearance RGB preservation',
    tieBreaking: 'earlier index priority',
    maxComparisonsGuard: 50000000,
    settingsVersion: 1,
    jsonMetadata: 'processing.palette with inputColors, usedColors, distance'
  },
  tests: {
    unitTests: 'scripts/palette-check.mjs (100% passed)',
    uiTests: 'scripts/pal001-ui-check.mjs (100% passed)',
    securityChecks: 'scripts/security-check.mjs (100% passed)',
    settingsChecks: 'scripts/settings-check.mjs (100% passed)',
    visualQuality: 'scripts/visual-quality-check.mjs (12/12 deterministic)'
  },
  sampleEnvelope,
  sampleResultJson
};

writeFileSync(resolve(evidenceDir, 'summary.json'), JSON.stringify(summaryData, null, 2), 'utf8');

const readmeContent = `# PAL-001 — Custom Palette Implementation & Verification Evidence
...
- **CSP SHA-256:** \`${summaryData.cspHash}\`
...
\`\`\`json
${JSON.stringify(sampleResultJson, null, 2)}
\`\`\`
...
writeFileSync(resolve(evidenceDir, 'README.md'), readmeContent, 'utf8');
