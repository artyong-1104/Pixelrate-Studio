...
| 구현 | 15 | 15 | 100% |
| QA | 10 | 15 | 66.7% |
| 현재 소스 해시 결속 브라우저 QA | 2 | 15 | 13.3% |

## 최근 작업 현황

- `2026-08-30`: GEO-001을 현재 HTML `d627799749e5e88e2490d246492dfcad59a7ff06511f899a667f5bfcd0ab2fc3`, Worker `444db75f4ebf6a57c43e1dc1805fb70459831003d34998a89b4db690994665f0`, git HEAD `3463fa01e21f6021430480f2e16bb42ca596bf36`에 결속해 재검증했다. 데스크톱·390px 모바일·1×/8×·sheet·invalid·Enter/Space QA, JPEG 디코딩·SHA-256, fail-closed 음성 테스트와 GEO evidence gate가 통과했다.
- GEO-001의 HTML 변경으로 기존 PERF-001 증거가 stale해져 동일한 현재 HTML·Worker 해시에 재결속했다. click 취소 `1.5ms`, Chrome Space 취소 `20.3ms`, 부분 결과 0, 전체 `scripts/*-check.mjs` `32/32`가 통과했다.
- 엄격한 `DONE` 증거가 남은 항목은 `OUT-001`, `CFG-001`, `UX-001`, `PAL-001`, `ALP-001`이다. 구현과 현재 자동 회귀는 통과하지만 명세가 요구하는 localhost 브라우저 QA·캡처·현재 소스 해시 결속 증거가 없어 `NEEDS_REVIEW`를 유지한다.
- 현재 작업은 커밋 `3463fa0` 위의 미커밋 변경으로 존재한다. 이 대시보드 갱신은 커밋·배포 완료를 의미하지 않는다.
...
| GEO-001 | 비율 유지 exact-factor/native 출력 | SOURCE-BACKED | P1 | QLT-001 | Luna xhigh | `DONE` | 완료 — 현재 HTML·Worker·HEAD 결속, desktop·390px 모바일·1×/8×·sheet·invalid·Enter/Space QA, 캡처 무결성·음성 테스트·GEO gate·전체 32/32 회귀 통과 | [열기](items/geo-001-exact-factor-output.md) | [검사·브라우저 QA 보고서](../evidence/geo-001/README.md), [측정 JSON](../evidence/geo-001/browser-qa.json) |
| OUT-001 | native + nearest 확대 이중 출력 | SOURCE-BACKED | P1 | GEO-001 | Luna xhigh | `NEEDS_REVIEW` | 자동 회귀 PASS, localhost 다운로드·ZIP·재업로드·모바일·키보드·제한 초과 QA와 현재 소스 결속 증거 필요 | [열기](items/out-001-dual-export.md) | [기존 자동 검사·산출물](../evidence/out-001/README.md) |
| CFG-001 | versioned 설정 JSON·preset | SOURCE-BACKED + ENGINEERING-INFERENCE | P1 | QLT-001 | Luna xhigh | `NEEDS_REVIEW` | 자동 회귀 PASS, preset 적용/취소·invalid import·legacy 복원·모바일·focus QA와 현재 소스 결속 증거 필요 | [열기](items/cfg-001-settings-presets.md) | [기존 자동 검사·예제](../evidence/cfg-001/README.md) |
| UX-001 | 1×/2×/8× 및 원본/결과 A/B | SOURCE-BACKED | P1 | GEO-001 | Luna xhigh | `NEEDS_REVIEW` | 자동 회귀 PASS, desktop/mobile A/B·로그 fallback·pan·Escape·focus·1×/2×/8× QA와 현재 소스 결속 증거 필요 | [열기](items/ux-001-actual-size-ab-preview.md) | [기존 자동 검사](../evidence/ux-001/README.md) |
| PAL-001 | custom palette 입력·미리보기 | SOURCE-BACKED | P1 | QLT-001, CFG-001 | Luna xhigh | `NEEDS_REVIEW` | 자동 회귀 PASS, palette 입력·chip·invalid·clear·dark/mobile/keyboard QA와 현재 소스 결속 증거 필요 | [열기](items/pal-001-custom-palette.md) | [기존 자동 검사](../evidence/pal-001/README.md) |
| ALP-001 | 고채도 배경과 alpha island 진단 | SOURCE-BACKED + ENGINEERING-INFERENCE | P1 | QLT-001 | Luna xhigh | `NEEDS_REVIEW` | 자동 회귀 PASS, 배경 순환·custom·island overlay·1×/2×/8×·mobile/focus QA와 현재 소스 결속 증거 필요 | [열기](items/alp-001-alpha-diagnostics.md) | [기존 자동 검사](../evidence/alp-001/README.md) |
...
| PERF-001 | 계측, Worker, 조건부 WASM | ENGINEERING-INFERENCE | P3 | QLT-001 및 대상 알고리즘 | Sol xhigh | `DONE` | 완료 — 현재 application·Worker 해시 재결속, QLT provenance·byte-identical gate, click 1.5ms·Chrome Space 20.3ms 취소·부분 결과 0, 256K/1M/4M·탭·모바일·Worker 실패 QA, trace·cancel MP4·전체 32/32 회귀 통과; WASM gate 미진입 | [열기](items/perf-001-worker-wasm.md) | [검사·브라우저 QA 보고서](../evidence/perf-001/README.md), [QLT fixture manifest](../evidence/perf-001/browser-fixtures/manifest.json), [기준선·Worker 비교](../evidence/perf-001/benchmark-report.json), [performance trace](../evidence/perf-001/performance-trace.json), [cancel MP4](../evidence/perf-001/browser-cancel-4m.mp4), [브라우저 QA](../evidence/perf-001/browser-qa.json) |
