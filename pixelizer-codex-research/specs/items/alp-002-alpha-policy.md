...
- palette sampling은 RGB contribution과 sample weight에 alpha/255를 적용한다. hidden RGB(alpha<threshold)는 제외한다.
...
- alpha matrix 추가 전 예상 JSON 크기를 검사하고 기존 저장 상한을 넘으면 로그 저장을 생략하되 다운로드는 허용 여부를 명확히 한다.
